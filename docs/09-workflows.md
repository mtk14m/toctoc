...
**État du code** (`services/closing.ts`, `jobs/closing-worker.ts`) : un passage BullMQ par minute ferme les liens `SPLIT` dont `orderCutoffTime` plus 2 minutes est passé, puis transmet les récaps. Trois choix à connaître. **Les liens `HOST_PAYS` ne sont pas encore traités** : leurs commandes sont « en attente » par conception, et la charge unique du créateur reste à construire — les fermer aujourd'hui annulerait tout le monde à tort. **Les `Payment` en attente ne sont pas modifiés** à la clôture : si l'opérateur confirme un débit après coup, le paiement est reconnu (`PAYMENT_TOO_LATE`) et loggé pour remboursement, au lieu d'être perdu. **L'envoi du récap est « au moins une fois »** : `GroupOrder.partnerNotifiedAt` ne se pose qu'après un envoi réussi, donc un envoi qui échoue est réessayé à chaque passage (un doublon vaut mieux qu'un partenaire qui ne cuisine pas). Le rappel avant l'heure limite (`reminderWorker`) n'est pas encore codé.
