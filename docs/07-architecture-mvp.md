...
| Rate limiting manuel Redis (`INCR` + `EXPIRE`) sur les endpoints sensibles, en plus du rate-limit global | `routes/auth.ts` (OTP, login) | Appliqué à la création de lien, aux tentatives de paiement, et à la saisie du code de confirmation de livraison (5 essais max, voir [09-workflows.md](09-workflows.md)) — même pattern partout, pas un mécanisme différent par cas d'usage |
...
| Temps réel avec **Socket.io** (pas de WebSocket brut), adaptateur Redis pour pouvoir scaler à plusieurs instances, auth du socket par JWT dans un middleware `io.use`, diffusion par "room" | `websocket/handler.ts`, `websocket/events.ts` | Réutilisé pour la liste qui se remplit en direct — voir mapping ci-dessous |
