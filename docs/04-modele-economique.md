...
| Coût d'une session livreur (créneau 11h–14h30) | ~60 000 à 80 000 GNF, revu à la hausse (voir ci-dessous) | Coût réel à négocier avec un premier livreur pilote, et confirmation que ce créneau est ou non une heure de forte demande pour un taxi-moto |

**Correction importante — notre première estimation (40 000 GNF) était probablement sous-évaluée.** Un conducteur de taxi-moto à Conakry évalue son gain à environ 200 000 GNF par jour sur une journée complète de travail. Sur trois heures (le créneau du déjeuner), même en prenant une moyenne horaire prudente et sans supposer que le déjeuner soit une heure de pointe, l'ordre de grandeur tourne plutôt autour de **60 000 à 80 000 GNF** que 40 000 — et il faut probablement une prime au-dessus de ce seuil, pas juste l'égaler : un livreur qui accepte un créneau fixe renonce aussi à la liberté de choisir ses courses et ses zones, ce qui a une valeur en soi. **Aucun livreur ne travaillera pour TocToc si la proposition est moins bonne que ce qu'il gagne déjà en taxi-moto libre** — c'est une contrainte dure, pas un détail d'ajustement.

**Le bon niveau de calcul n'est pas "par immeuble", c'est "par session de livreur".** Le plan de Phase 0 ([03-roadmap-phases.md](03-roadmap-phases.md)) prévoit un ou deux livreurs pour deux ou trois immeubles — un livreur en session peut donc enchaîner plusieurs `GroupOrder` dans la même fenêtre de 3 heures, pas un seul. Recalculé sur cette base, avec 3 immeubles à 8 commandes chacun (24 commandes au total) sur la même session : frais de livraison collectés ≈ 3 × 39 000 = 117 000 GNF ; commissions ≈ 24 × 3 750 = 90 000 GNF ; revenu total ≈ 207 000 GNF ; coût de la session (haut de fourchette) = 80 000 GNF ; **marge ≈ 127 000 GNF sur la session, avant incitatif au relais**. L'économie tient à l'échelle d'une session multi-immeubles, même avec un coût livreur réévalué à la hausse — mais elle ne tiendrait plus si un seul livreur ne servait qu'un seul petit immeuble à la fois, ce qui recadre l'objectif de Phase 0 : viser la couverture de plusieurs immeubles par livreur, pas seulement le volume d'un immeuble isolé.
...
| **Session garantie, engagement court (recommandé)** | Un montant fixe garanti pour être disponible sur le créneau 11h–14h30, qu'il y ait 5 ou 25 commandes ce jour-là — sans contrat de travail long terme, révisable jour par jour ou semaine par semaine tant que le volume n'est pas stabilisé. C'est le compromis qui donne au livreur la prévisibilité qu'il recherche sans engager TocToc sur un salariat prématuré. C'est aussi, dans l'esprit, ce que Chowdeck a fait à ses débuts : une petite équipe de livreurs suivie de près par les fondateurs, pas un statut figé dès le premier jour — voir [10-benchmark-produit-mondial.md](10-benchmark-produit-mondial.md). |
