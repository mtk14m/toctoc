...
- **Source de vérité : `platform/apps/api/prisma/schema.prisma`.** Le bloc ci-dessus est le raisonnement de conception ; en cas d'écart, c'est le fichier du dépôt qui fait foi. Écarts déjà assumés : Prisma 7 (l'URL de la base vit dans `prisma.config.ts`, plus dans le schéma), un index sur `Payment.providerTransactionId` (utile quand plusieurs `Payment` partagent une transaction, en `HOST_PAYS` ; en `SPLIT`, le webhook retrouve le paiement par **notre** id, envoyé à l'opérateur comme référence et renvoyé tel quel, ce qui évite la course avec l'enregistrement de son propre identifiant), un champ `GroupOrder.partnerNotifiedAt` (posé quand le récap a été transmis au partenaire : tant qu'il est vide, la clôture réessaie l'envoi), sur `Partner` les heures de service (`serviceStartMinute`, `serviceEndMinute`, en minutes depuis minuit, 9h - minuit par défaut) et la présentation de l'annuaire public (`description`, `logoUrl`, `coverUrl`, `tags`), et des index sur `GroupOrder.creatorId`, `OrderItem.userId` et `Delivery(driverId, status)`.
...
- **Tous les montants sont des `Int`, jamais des `Float` (décidé au démarrage du développement).** Le GNF n'a pas de sous-unité, et `Float` accumule des erreurs d'arrondi dès qu'on additionne des paiements (un `Payout` agrégé sur une semaine ne doit jamais avoir un franc d'écart avec la somme des `Payment`). Seuls restent en `Float` : `Partner.commissionRate` (un ratio, pas un montant) et les coordonnées GPS. Conséquence à l'implémentation : l'arrondi est explicite, à un seul endroit (`services/pricing`), et testé. **Piège découvert au développement** : `Math.round(unitPrice × quantity × commissionRate)` est faux sur des flottants (`1500 × 0.145` vaut `217.4999…`, donc 217 au lieu de 218) — la commission se calcule en points de base entiers (`taux × 10 000`, puis division entière arrondie), avec un taux à 4 décimales maximum. Un test (`tests/lib/schema.test.ts`) empêche de réintroduire un `Float` monétaire par inadvertance.
...
- **Le mode solo, et l'extensibilité vers la Phase 3, sans migration** : une commande individuelle n'est qu'un `GroupOrder` que personne d'autre ne rejoint — même `OrderItem`, même `Payment`, même `Delivery`. Dès la Phase 1 le solo suit le même cycle que les groupes (commande ouverte 20 minutes, puis livraison) ; en Phase 3 la livraison à la demande change le produit (plus de fenêtre d'ouverture), pas le schéma. Un `GroupOrder` porte toujours **un seul** `partnerId` : deux restaurants, c'est deux `GroupOrder`.
