...
- **Le mode solo, et l'extensibilité vers la Phase 3, sans migration** : une commande individuelle n'est qu'un `GroupOrder` que personne d'autre ne rejoint — même `OrderItem`, même `Payment`, même `Delivery`. Dès la Phase 1 le solo est livré au même créneau que les groupes ; en Phase 3 la livraison à la demande change le produit (plus de créneau fixe), pas le schéma. Un `GroupOrder` porte toujours **un seul** `partnerId` : deux restaurants, c'est deux `GroupOrder`.
